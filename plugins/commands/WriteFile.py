...
class WriteFile(BasePlugin):
    ...
    def run_plugin(self, arguments: dict[str, any]):
        ...
        if not os.path.isfile(os.path.join(os.environ["AGENT_WORKSPACE"], arguments["file_name"])):
            with open(os.path.join(os.environ["AGENT_WORKSPACE"], arguments["file_name"]), "w") as f:
